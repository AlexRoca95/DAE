...
#pragma region textureImplementations
	...
	bool TextureFromFile(const std::string& path, Texture& image)
	{
		...
		TextureFromSurface(pLoadedSurface, image);
	...
	bool TextureFromString(const std::string& text, const std::string& fontPath, int ptSize, const Color4f& textColor, Texture& image)
	{
		// Create font
		TTF_Font* pFont{};
		...
		bool textureOk = TextureFromString(text, pFont, textColor, image);
		TTF_CloseFont(pFont);

		return textureOk;
	}

	bool TextureFromString(const std::string& text, TTF_Font* pFont, const Color4f& color, Texture& image)
	{
		//Render text surface
		SDL_Color textColor{};
	...
	void TextureFromSurface(const SDL_Surface* pSurface, Texture& image)
	{
		//Get image dimensions
		image.width = float(pSurface->w);
		image.height = float(pSurface->h);
	...
	void DeleteTexture(Texture& image)
	{
		glDeleteTextures(1, &image.id);
	}

	void DrawTexture(const Texture& image, const Point2f& dstBottomLeft, const Rectf& srcRect)
	{
		Rectf dstRect{ dstBottomLeft.x, dstBottomLeft.y, srcRect.width, srcRect.height };
		DrawTexture(image, dstRect, srcRect);
	}


	void DrawTexture(const Texture& image, const Rectf& dstRect, const Rectf& srcRect)
	{
		...
		if (!(srcRect.width > 0.0f && srcRect.height > 0.0f)) // No srcRect specified
		{
			...
			defaultDstHeight = image.height;
			defaultDstWidth = image.width;
		}
		else // srcRect specified
		{
			// Convert to the range [0.0, 1.0]
			textLeft = srcRect.left / image.width;
			textRight = (srcRect.left + srcRect.width) / image.width;
			textTop = (srcRect.bottom - srcRect.height) / image.height;
			textBottom = srcRect.bottom / image.height;
