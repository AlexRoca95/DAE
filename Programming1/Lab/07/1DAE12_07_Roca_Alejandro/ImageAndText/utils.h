...
#pragma region TextureFunctionality
	...
	struct Texture
	{
		GLuint id;			// Unsigned integer variation type (is from OpenGL)
		float width;
		float height;
	};
	bool TextureFromFile(const std::string& path, Texture& texture);  // Succes loading or not the file
				 // Path can be absolute or relative
				// Absolute --> Path in your computer (if you put it in other computer it wont work)
				// Relative --> Relative to the solution that you are working.
	...
	void DrawTexture(const Texture& texture, const Rectf& dstRect, const Rectf& srcRect = {});
		// Put the texture inside a rectangle size. Scale the texture acording to the rect
	void DeleteTexture(Texture& texture);
		// Texture deleted from the GPU memory
