...
void InitShips()
{
	...
	carrier.color.r = 0.15f;
	carrier.color.g = 0.63f;
	carrier.color.b = 0.56f;
	InitShip(g_pPlayerArrayShips, carrier, 5, 0);  // Ship, size and index of the array
	
	// Battleship
	Ship battleship{};
	battleship.text = g_Ships;
	battleship.color.r = 0.15f;
	battleship.color.g = 0.63f;
	battleship.color.b = 0.56f;
	InitShip(g_pPlayerArrayShips, battleship, 4, 1);

	// Destroyer
	Ship destroyer{};
	destroyer.text = g_Ships;
	destroyer.color.r = 0.15f;
	destroyer.color.g = 0.63f;
	destroyer.color.b = 0.56f;
	InitShip(g_pPlayerArrayShips, destroyer, 3, 2);

	// Submarine
	Ship submarine{};
	submarine.text = g_Ships;
	submarine.color.r = 0.15f;
	submarine.color.g = 0.63f;
	submarine.color.b = 0.56f;
	InitShip(g_pPlayerArrayShips,submarine, 3, 3);

	// Patrol Boat
	Ship patrolBoat{};
	patrolBoat.text = g_Ships;
	patrolBoat.color.r = 0.15f;
	patrolBoat.color.g = 0.63f;
	patrolBoat.color.b = 0.56f;
...
bool DrawEnemyMove(GridCell* grid, int idx)
{
	...
	else
	{
		...
		{
			// Miss
			utils::SetColor(0.16f, 0.66f, 0.90f);
			EnemyMove = true;
		}
	}
...
void DrawShipInGrid(GridCell* grid, int idx, int rowIdx, int colIdx)
{
	...
	for (int i{ 0 }; i < grid[idx].shipInfo.size; i++)
	{
		...
		{
			// Ship color
			utils::SetColor(grid[idx2].shipInfo.color);
		}
...
void DrawShipPlacement()
{
	...
	utils::SetColor(g_pPlayerArrayShips[g_ShipsIndex].color);
